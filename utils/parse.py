...
def process(filename: str) -> dict[str: str, str: list[str]]:
    ...
    while i < len(raw_lines):
        line = raw_lines[i]
        if _is_topset(line) or _is_warmup(line):
            succ = raw_lines[i+1]
...
def get_name(line: str) -> str:
    ...
    for word in line.split():
        if "take" in word or "warm" in word or _has_digit(word):
...
def get_weight(line: str) -> [str, float, tuple[float, float]]:
    ...
    for word in line.split():
        if word.endswith("lbs"):
            ...
            if m is not None:
                return _convert(m.group(1).removesuffix("lbs"), float)
...
def get_specs(line: str):
    ...
    if m is None:
        ...
        count = _convert(m.group(1), int)
        reps = _convert(m.group(2), int)
    return count, reps
...
def get_parse(lowercase: str):
    ...
    """
    if _is_topset(lowercase):
        return _parse_topset
    if _is_warmup(lowercase):
        return _parse_warmup
    return _parse_simple


def _is_warmup(line: str) -> bool:
    return "warm" in line


def _is_topset(line: str) -> bool:
    """
    >>> _is_topset("barbell back squat build up to a top set of 3 reps, 4 from failure. then do 3x5 backoff sets -18-22% off the top set weight. goal for these is 4-5 reps from failure so pick weight accordingly.")
    True
    """
    return "top set" in line


def _is_homoousian(line: str) -> bool:
    """
    ...
    >>> _is_homoousian("(probably somewhere between 90-110lbs for top set based on how you're feeling tomorrow.)")
    True
    """
...
def _parse_topset(line: str) -> list[Set_]:
    ...
    words = line.split()
    top.reps = _convert(_get_first(_has_digit, words), int)
    weights = re.findall(INTERVAL + "lbs", line)
    top.weight = _convert(weights[0], float)
    top.count = 1
    sets.append(top)
    ...
    backoff = Set_()
    if "%" in line:
        percent = _convert(_get_first(lambda s: "%" in s, words).removesuffix("%"), float)
...
def _parse_warmup(line: str) -> list[Set_]:
    # exclude everything before warmup (e.g. exercise name) to ignore edge cases (e.g. '2ct')
    words = line.split()
    ...
    sets = []
    parts = [line for line in line.split(",") if line and _has_digit(line) and _has_weight(line)]
    for part in parts:
...
def _convert(s: str, cls: Type):
    """
    >>> _convert('4', int)
    4
    >>> _convert('4-5', int)
    (4, 5)
    >>> _convert('-10-15', float)
    (-10.0, -15.0)
    """
    if s.isnumeric():
        return cls(s)
...
def _get_first(func, iterable):
    for elem in iterable:
        if func(elem):
            return elem


def _has_weight(s: str) -> bool:
    return any(substring in s for substring in ["bar", "lbs", "bodyweight"])


def _has_interval(s: str) -> bool:
